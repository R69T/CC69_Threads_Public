...
여러 번 번거롭게 테스트해 주신 모든 분들께 진심으로 감사드립니다. 실제 사용 중 발견한 문제를 계속 알려 주신 점에도 감사드립니다.

CC69는 현재 다음과 같은 알려진 문제를 수정했습니다.

- **대형 계정에서 팔로워 수와 팔로잉 수가 많을 때 빠른 검사 결과가 완전히 수집되지 않는 문제.**
- **관계 데이터를 일부만 가져온 뒤 프로그램이 오랫동안 아무 동작도 하지 않거나 멈춘 것처럼 보이는 문제.**
- **대규모 Followers / Following 목록의 마지막 부분을 완전히 가져오지 못하면 정리 작업을 계속할 수 없던 문제.**

최신 버전은 위 상황을 중심으로 조정 및 수정되었습니다. 앞으로 안정성을 더 높이기 위해 계정 규모가 다른 다양한 실제 환경의 테스트가 계속 필요합니다.

**CC69에 한 번 더 기회를 주시고, 계속 사용하고 테스트해 주시며 문제가 있다면 피드백을 보내 주세요. 여러분의 모든 의견이 CC69를 더 안정적으로 만드는 데 큰 도움이 됩니다. 감사합니다.**
