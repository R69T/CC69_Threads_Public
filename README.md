...
CC69 Threads Manager 是 Windows 桌面工具，主要幫你整理 Threads 的 Following / Followers 關係：
...
> 公開頁面中的實際宣傳截圖會以目前正式程式介面為準，不再使用與實際 UI 不一致的示意操作圖。
...
| 步驟 | 要做什麼 | 你會看到什麼 |
| --- | --- | --- |
| **1. 啟動 CC69** | 解壓縮 ZIP 後執行 `CC69_Threads_Manager.exe` | CC69 會先顯示啟動／初始化畫面，再進入主介面。 |
| **2. 確認 Threads 登入** | 按藍色「確認 Threads 登入」 | 在 Threads / Meta 官方頁面完成登入。 |
| **3. 開始快速檢查** | 登入成功後按「開始快速檢查」 | 進度條顯示 Followers / Following 已取得數量。 |
| **4. 查看結果** | 查看上方統計與下方表格 | 追蹤中、粉絲、沒回追、我沒回追、互追。 |
...
- 🕒 **新追蹤保護**：預設 **0 天＝不保護**。例如設為 7 天，最近 7 天才追蹤的帳號不會列入「沒回追」退追整理。
...
新版會依 Windows 目前螢幕的**可用工作區與顯示縮放比例**自動決定第一次開啟的視窗大小。

- 小型／高縮放螢幕：上方資訊區自動壓縮。
- 一般 1080p：盡可能一次完整呈現主要操作與結果區。
- 較大螢幕：新增高度優先提供給下方帳號結果表格。
...
CC69 目前**尚未使用商業 Windows Code Signing 憑證**簽署 EXE，所以 Windows 可能顯示「Windows 已保護您的電腦」或「未知的發行者」。
...
與 ZIP 內的 `CC69_Threads_Manager.exe.sha256` 比對即可。

---

## 🔐 登入與隱私

Threads / Meta 密碼是在**官方登入頁面**輸入，不是在 CC69 自己的介面輸入。

第一次登入後如果 Threads 顯示「儲存你的登入資料？」流程，CC69 會嘗試完成「儲存資料」，讓之後通常不需要每次重新登入。
...
如果快速檢查仍不完整，切換到 **完整資料檢查**，再匯入 Meta / Threads 官方下載資料。

---

## ⏳ 取消追蹤需要多久？

CC69 會**逐一開啟並確認每個帳號**，因此選取帳號越多，處理時間越長。

開始後請耐心等待，並查看程式內的取消追蹤進度。不要因為處理時間較長就重複啟動相同操作。
